...
| 2026-09-03 | v1.2 | F-9-0 spike 实测：claude-agent-acp 对「拆解执行计划」类 prompt 本轮**未发 plan block**。按 §8 降级预案——计划栏「无数据源即隐藏」，渲染层（组件+纯函数+单测）已交付，待 harness 升级自动点亮（AC-P9-0-1 以降级记录通过） | 所有者+Claude |
